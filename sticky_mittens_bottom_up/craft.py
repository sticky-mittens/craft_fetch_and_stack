...
import torch
import random # for randomly setting positions to empty spot
...
class CraftState(object):
  def __init__(self, scenario, grid, pos, dir, inventory, steps_since_last_learning_action=1):
    ...
    # self.steps_since_last_learning_action = steps_since_last_learning_action # new for bottom up: 
  ...
  def randomly_set_position_to_empty_spot(self):
    empty_positions= []
    for x in range(WIDTH):
      for y in range(HEIGHT):
  ...
  # def play_option(self, thing_name):
  #   if DEBUG: print('[DEBUG] playing option for thing={}.'.format(thing_name))
  #   actor_network_file = "./models/actor_" + thing_name + "_network.pkl"
  #   critic_network_file = "./models/critic_" + thing_name + "_network.pkl"
  #   policy_info_file = "./models/policy_info_" + thing_name + ".json"
  ...
  #   def accomplished(thing_name):
  #     index = self.world.cookbook.index[thing_name]
  ...
  #   max_option_steps = 500
  #   while (steps_for_option_execution != max_option_steps) and (not accomplished(thing_name)):
  #     current_state = self.features().astype(np.float32)
  #     state_length= self.world.n_features
  ...
  #   if DEBUG: print('[DEBUG] Is thing={} in inventory, a.k.a. did option accomplish sub_task? ANS: {}'.format(thing_name, accomplished(thing_name)))
  #   return reward, state, steps_for_option_execution
  ...
  def step(self, action):
    ...
    # # uber actions / option templates
    
    # elif action.find('give_') != -1:
    #     if DEBUG: print('[DEBUG] Uber action is ', action)
    ...
    #         if DEBUG: print('[DEBUG] Amount of thing id {}, i.e. thing {}, in inventory is '.format(thing_id, thing), self.inventory[thing_id])
    ...
    #         reward, new_state, steps_since_option_was_called = self.play_option(thing)
    #         new_state.steps_since_last_learning_action = steps_since_option_was_called # new for bottom up: overriding k=1
    #         return reward, new_state
