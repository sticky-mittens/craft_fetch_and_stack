...
class EnvironmentFactory(object):
  ...
  def __init__(self,
    ...
    with open(hints_path) as hints_f:
      self.hints = yaml.load(hints_f, yaml.Loader)
  ...
  def _create_environment(self, task_name, start_cell = None):
    # Get the task
    task = self.tasks[task_name]
    goal_arg = task.goal[1]

    # Sample a world (== scenario for them...)
    scenario = self.world.sample_scenario_with_goal(goal_arg, start_cell)
  ...
  def sample_environment(self, task_name=None, start_cell = None):
    if task_name is None:
      task_name = np.random.choice(self.task_names)

    if self._reuse_environments:
      return self._env_cache.setdefault(task_name,
                                        self._create_environment(task_name, start_cell))
    else:
      return self._create_environment(task_name, start_cell)
