...
def compute_return(trajectories, gamma = 0.99):
    ...
    # EDIT
    map_of_discounts = {'gold':100, 'bridge':50, 'wood':25, 'iron':25, 'gem':100, 'axe':50, 'stick': 40} # prev: 'axe': 50, 'stick': 40
    trajectory_values = {}
    for trajectory_index in trajectories.keys():
        ...
        for step_index in reversed(range(no_of_steps)):
            ...
            if trajectory[step_index]['is_uber']:
                uber_thing = trajectory[step_index]['uber_thing']
                #print('[DEBUG] Discounting more by \gamma^{} for uber thing {}.'.format(map_of_discounts[uber_thing], uber_thing))
                discount_factor = gamma ** map_of_discounts[uber_thing]
            else:
                discount_factor = gamma
            return_val = reward + discount_factor * return_val # change to gamma^k when it is an uber action
